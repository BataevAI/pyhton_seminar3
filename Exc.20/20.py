...
dictionary2 = {
 ...
 'Ф': 10,'Щ': 10,'Ъ': 10, }
# ----------------------------
print("Введите слово, а мы посчитаем его \"стоимость\" ")
word1 = str(input())
word = word1.upper()
count = 0

for i in range(len(word)):
    if dictionary1.get(word[i]) != None:
        # print(f'word[i]: {word[i]}')
        # print(f'dictionary1.get(word[i]): {dictionary1.get(word[i])}')  
        count = count + dictionary1.get(word[i])
    else:
        # print(f'word[i]: {word[i]}')
        # print(f'dictionary1.get(word[i]): {dictionary2.get(word[i])}')  
        count = count + dictionary2.get(word[i])

print(f'Цена слова \"{word1}\" составляет {count} очков') 
